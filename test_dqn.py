...
print(epoch_total_reward_energy_ep)
...
# stci = np.sum(epoch_CA_intime) #short term correct intime
# stcl = np.sum(epoch_CA_late) #short term correct late
# ltci = np.sum(epoch_UAC_intime) #long term correct intime
# ltcl = np.sum(epoch_UAC_late) #long term correct late
# ci = np.sum(epoch_CI) #correct inactions
# ii = np.sum(epoch_II) #incorrect inactions
# ui = np.sum(epoch_UAI_intime) #unnec intime
# ul = np.sum(epoch_UAI_late) #unnec late
# iai = np.sum(epoch_IA_intime) #incorrect intime
# ial = np.sum(epoch_IA_late) #incorrect late
...
# if env.test: fig1.savefig(save_path+'/00_TEST_ACTIONS_PIE_mean_epoch.jpg')
# else: fig1.savefig(save_path+'/00_TRAIN_ACTIONS_PIE_mean_epoch.jpg')




# stcl = np.sum(epoch_CA_late[best_time_position]) #short term correct late
# ltci = np.sum(epoch_UAC_intime[best_time_position]) #long term correct intime
# ltcl = np.sum(epoch_UAC_late[best_time_position]) #long term correct late
# ci = np.sum(epoch_CI[best_time_position]) #correct inactions
# ii = np.sum(epoch_II[best_time_position]) #incorrect inactions
# ui = np.sum(epoch_UAI_intime[best_time_position]) #unnec intime
# ul = np.sum(epoch_UAI_late[best_time_position]) #unnec late
# iai = np.sum(epoch_IA_intime[best_time_position]) #incorrect intime
# ial = np.sum(epoch_IA_late[best_time_position]) #incorrect late
...
# if env.test: fig1.savefig(save_path+'/00_TEST_ACTIONS_PIE_best_epoch.jpg')
# else: fig1.savefig(save_path+'/00_TRAIN_ACTIONS_PIE_best_epoch.jpg')
...
plt.plot([48020,48202,48529], 'o-',label='Interaction')
plt.axhline(y=48738, color='k', label='Reactive HRI')
...
with open(path +'/best_epoch_test_iter_time.txt', 'w') as f: f.write(file_name.split('/')[-1])

print(epoch_total_time_interaction)
if env.test: 
